...
export function Hero({
  ...
  const primaryCta = settings.hero_primary_cta_text || "View My Work";
  const primaryHref = settings.hero_primary_cta_href || "#projects";
  const secondaryCta = settings.hero_secondary_cta_text || "Let's Connect";
  const secondaryHref = settings.hero_secondary_cta_href || "#contact";
      ...
      <Container className="relative grid items-center gap-10 lg:grid-cols-[minmax(0,1.1fr)_minmax(280px,0.9fr)] lg:gap-12">
        <div>
          ...
          <h1 className="hero-rise max-w-[12ch] text-[clamp(3.1rem,9.2vw,7.25rem)] font-medium leading-[0.96] tracking-[-0.06em] [animation-delay:80ms]">
            <HeroName title={settings.hero_title} />
          </h1>
